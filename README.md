...
후기를 쓰기 전, 이 말을 먼저 하고 싶다. "구글 짱!" . 처음 GMS를 접할때는 뭐 이리 설명이 불친절해! 라는 반응이 먼저 나왔다. 솔직히 잘 이해도 되지 않았고 쓰기 힘든 면이 없지 않아 있었다. 하지만... 국산 Map API(다음, 네이버)의 가이드는 어우... 더 심했기에 구글이 더 사랑스러웠다! 딜리케이트 메소드도 잘 되어 있었고 전체적으로 만족한다. 구글 맵스 말고도 플레이스 및 고도 데이터 등 전체적으로 풍부하게 제공되어있기에 다음 프로젝트에서도 활용할 생각이다.(초딩일기처럼 보인다면, 맞습니다...)
